	...
	return (
		<Panel>
			<div style={{ display: 'flex', justifyContent: 'space-between', flex: 1, paddingBottom: 0, gap: 1 }}>
				...
				{system.settings.horizontalLayout ? trailingButtons : leadingButtons}

				<div style={{
					display: 'flex', flex: 1, justifyContent: 'left', padding: '4px 3px 0px 7px',
					color: system.settings.colors.buttonColor, flexDirection: 'column', gap: 1
				}}>
					<h3>{files.currentDocument?.folderName}/{files.currentDocument?.name}</h3>
					<div>{utils.textStats(files.currentDocument)}</div>
				</div>

				{system.settings.horizontalLayout ? leadingButtons : trailingButtons}
